...
func TestMaintenanceReleaseOrchestratorForcesCredentialMigrationOrder(t *testing.T) {
	...
	for _, required := range []string{"maintenance", "old/new binary overlap is forbidden", "restore", "agentnexus_release_preflight_bin", "agentnexus_release_start_new_isolated_hook", "agentnexus_release_stop_new_hook", "agentnexus_release_open_traffic_hook"} {
		if !strings.Contains(script, required) {
			t.Fatalf("release script missing fail-closed guard %q", required)
		}
	}
	makefile, err := os.ReadFile(filepath.Join("..", "..", "Makefile"))
	if err != nil || !strings.Contains(string(makefile), "go build ./cmd/release-preflight") {
		t.Fatalf("release preflight command is not part of the build contract: %v", err)
	}
}
